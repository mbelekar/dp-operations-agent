...
122/122 offline tests pass with no live dependency (Kafka, Flink, Marquez, dbt, or Anthropic). Args pass through, so `./auto/test -m llm` also runs the full loop against a live model.
