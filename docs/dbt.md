...
By Phase 3, the agent covers all four systems in scope (Kafka, Flink, dbt, data quality) and gains the ability to propose a remediation, not just diagnose. `evidence/schema.py`'s `Proposal` type already exists (reserved since Phase 1) but stays unused until this phase populates it. dbt is the layer where most alerts actually fire, a failing test, a stale source, even though the root cause usually lives upstream. That makes it the natural point to test the agent's core discipline: telling "the model's logic is wrong" apart from "the model is correct but its inputs are bad."
...
The key discipline this module has to encode: check whether the same test passed on the previous run with no code change to the model. If it did, the fault is almost certainly upstream, and the agent should route the investigation there via lineage instead of suggesting a model edit. That is exactly the trap a naive "the test on model X failed, so fix model X" agent would fall into.

## How it plugs into the existing architecture

Same pattern as Kafka and the Flink module:
...
- **`tools/dbt/live_gateway.py`** / **`tools/dbt/fixture_gateway.py`**: real vs. canned-snapshot implementations, mirroring the Kafka and Flink modules.
...
- **`tools/data_quality/`**: a related but separate module (schema drift, statistical anomalies, cross-system contract violations). It is mostly a signal source feeding the core loop, not a standalone diagnostic path. A data-quality alert is almost always a symptom, and the loop's job is to trace it to a Kafka, Flink, or dbt root cause via lineage.
...
Unlike Phase 2, which only adds a system, Phase 3 changes what `submit_diagnosis` accepts. A `Diagnosis` can now carry a `Proposal` (tier, action, expected outcome, rollback step, downstream consumers, idempotency rating). There is still no execution tool (that's Phase 4), so a proposal at this stage is informational: reviewed by a human, logged, and not acted on.
