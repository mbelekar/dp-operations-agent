...
def build_lineage_tools(
    ...
    async def walk_lineage_upstream(node_id: str) -> str:
        """Find what's upstream of a Kafka topic or Flink job, to trace a
        root cause from where an alert fired to a different system entirely.
        node_id must follow the convention: a Kafka topic is
        "dataset:kafka:{topic}", a Flink job is "job:flink:{job_name}". Call
